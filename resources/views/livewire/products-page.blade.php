        ...
        <div class="px-4 py-4 mx-auto max-w-7xl lg:py-6 md:px-6">
            <div class="flex flex-wrap mb-24 -mx-3">
                ...
                <div class="w-full pr-2 lg:w-1/4 lg:block">
                    @foreach (['Categories' => $categories, 'Brand' => $brands] as $title => $items)
                        <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                            <h2 class="text-2xl font-bold dark:text-gray-400">{{ $title }}</h2>
                            <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
                            @if($items->isEmpty())
                                <p class="text-gray-500 dark:text-gray-400">Aucune {{ strtolower($title) }} disponible.</p>
                            @else
                                <ul>
                                    @foreach ($items as $item)
                                        <li class="mb-4" wire:key="{{ $item->id }}">
                                            <label for="{{ $item->slug }}" class="flex items-center dark:text-gray-400">
                                                <input type="checkbox" wire:model.live="selected_{{ strtolower($title) }}"
                    ...
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                        ...
                        <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
                        <div class="font-semibold">{{ Number::currency($price_range, 'XAF') }}</div>
                        <div>
                            <input type="range"
                                   class="w-full h-1 mb-4 bg-blue-100 rounded appearance-none cursor-pointer"
                                   max="500000" value="300000" step="1000" wire:model.live="price_range">
                            <div class="flex justify-between">
                                <span class="inline-block text-lg font-bold text-blue-400">{{ Number::currency(1000, 'XAF') }}</span>
                                <span class="inline-block text-lg font-bold text-blue-400">{{ Number::currency(500000, 'XAF') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="border border-gray-300 dark:border-gray-700">
                                    ...
                                    <div class="flex justify-center p-4 border-t border-gray-300 dark:border-gray-700">
                                        <a wire:click.prevent="addToCart({{ $product->id }})"
                                           class="cursor-grab text-gray-500 flex items-center space-x-2 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-300">
